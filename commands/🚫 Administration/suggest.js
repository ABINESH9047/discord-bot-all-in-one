...
module.exports = {
  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    try {
      ...
      let cmdroles = client.settings.get(
        message.guild.id,
        "cmdadminroles.suggest"
      );
      var cmdrole = [];
      ...
      if (
        ...
        !cmdroles.includes(message.author.id) && [
          ...message.member.roles.cache.values(),
        ] &&
        !message.member.roles.cache.some((r) =>
          adminroles.includes(r ? r.id : r)
        ) &&
        !Array(message.guild.ownerId, config.ownerid).includes(
          message.author.id
        ) &&
        ...
        return message.reply({
          embeds: [
            new MessageEmbed()
              ...
              .setTitle(
                eval(
                  client.la[ls]["cmds"]["administration"]["suggest"][
                    "variable1"
                  ]
                )
              )
      ...
      client.settings.ensure(message.guild.id, {
        suggest: {
          channel: "",
          approvemsg: `<a:tick:1241004249920442429> Accepted Idea! Expect this soon.`,
          denymsg: `<a:no:1240848440473292810> Thank you for the feedback, but we are not interested in this idea at this time.`,
          maybemsg: `💡 We are thinking about this idea!`,
          duplicatemsg: `💢 This is a duplicated Suggestion`,
          soonmsg: `👌 Expect this Feature Soon!`,
          statustext: `<a:loding:1241084440172564480> Waiting for Community Feedback, please vote!`,
      ...
      switch (args[0]) {
        ...
        default:
          message.reply({
            content: eval(
              client.la[ls]["cmds"]["administration"]["suggest"]["variable25"]
            ),
          });
          break;
      }
      ...
      if (embed.fields[2]) {
        embed.fields[2].name ==
          `<a:arrow:1240881386773024788> __Reason by **${message.author.tag}**:__`;
        embed.fields[2].value == `>>> ${String(reason).substr(0, 1000)}`;
      } else {
        embed.addField(
          `<a:arrow:1240881386773024788> __Reason by **${message.author.tag}**__`,
          `>>> ${String(reason).substr(0, 1000)}`
        );
      }
      ...
      if (client.settings.get(message.guild.id, `adminlog`) != "no") {
        try {
          var channel2send = message.guild.channels.cache.get(
            client.settings.get(message.guild.id, `adminlog`)
          );
          if (!channel2send)
            return client.settings.set(message.guild.id, "no", `adminlog`);
          channel2send.send({
            embeds: [
              new MessageEmbed()
                .setColor(es.color)
                ...
                .setFooter(client.getFooter(es))
                .setAuthor(
                  `${require("path").parse(__filename).name} | ${
                    message.author.tag
                  }`,
                ...
                .addField(
                  ...
                  eval(
                    client.la[ls]["cmds"]["administration"]["ban"]["variable15"]
                  )
                )
                .addField(
                  eval(
                  ...
                  eval(
                    client.la[ls]["cmds"]["administration"]["ban"]["variable16"]
                  )
                )
                .setTimestamp()
                .setFooter(
      ...
      return message.reply({
        embeds: [
          new MessageEmbed()
            ...
            .setDescription(
              eval(
                client.la[ls]["cmds"]["administration"]["suggest"]["variable29"]
              )
            ),
        ],
      });
