...
module.exports = {
  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    try {
      ...
      let timeouterror;
      let NumberEmojiIds = getNumberEmojis().map(
        (emoji) => emoji?.replace(">", "").split(":")[2]
      );
      first_layer();
        ...
        collector.on("collect", (menu) => {
          if (menu?.user.id === cmduser.id) {
            ...
            if (menu?.values[0] == "Cancel")
              return menu?.reply(
                eval(
                  client.la[ls]["cmds"]["setup"]["setup-ticket"]["variable3"]
                )
              );
            menu?.deferUpdate();
            used1 = true;
          ...
          menumsg.edit({
            ...
            content: `${
              collected && collected.first() && collected.first().values
                ? `<a:tick:1241004249920442429> **Selected: \`${
      ...
      async function handle_the_picks(optionhandletype, menuoptiondata) {
        switch (
          ...
          case "Create Custom Command":
            {
              if (
                client.customcommands.get(message.guild.id, "commands").length >
                24
              )
                return message.reply({
                  embeds: [
                    new Discord.MessageEmbed()
                      ...
                      .setDescription(
                        `You cannot have more then **25** Custom Commands`.substr(
                          0,
                          2000
                        )
                      )
                            ...
                            return message.reply({
                              embeds: [
                                new Discord.MessageEmbed()
                                  ...
                                  .setColor(es.wrongcolor)
                                  .setDescription(
                                    `Cancelled the Operation!`.substr(0, 2000)
                                  )
                                  .setFooter(client.getFooter(es)),
                              ],
                            });
          ...
          case "Delete Custom Command":
            {
              ...
              if (!cuc || cuc.length < 1)
                return message.reply(":x: There are no Custom Commands");
              let menuoptions = [];
              cuc.forEach((cc, index) => {
                menuoptions.push({
                  value: `${cc.name}`.substr(0, 25),
                  description: `Delete ${cc.name} ${
                    cc.embed ? "[✅ Embed]" : "[❌ Embed]"
                  }`.substr(0, 50),
              ...
              let Selection = new MessageSelectMenu()
                ...
                .setMinValues(1) //OPTIONAL , this is how many values you need to have at each selection
                .setPlaceholder(
                  "Select all Custom Commands which should get deleted"
                )
                .addOptions(
                  menuoptions.map((option) => {
                    let Obj = {
              ...
              let MenuEmbed = new Discord.MessageEmbed()
                ...
                .setDescription(
                  `**Select all \`Custom Commands\` which should get __deleted__**`
                );
              //send the menu msg
              let menumsg = await message.reply({
                embeds: [MenuEmbed],
                ...
                if (menu?.user.id === cmduser.id) {
                  ...
                  for (const value of menu?.values) {
                    client.customcommands.remove(
                      message.guild.id,
                      (d) =>
                        String(d.name).substr(0, 25).toLowerCase() ==
                  ...
                  return message.reply({
                    embeds: [
                      new Discord.MessageEmbed()
                        .setTitle(
                          `Deleted ${menu?.values.length} Custom Commands!`
                        )
                        .setDescription(
                          `There are now \`${
                            cuc.length - menu?.values.length
                          } Custom Commands\` left!`
                        )
                        .setColor(es.color)
                        .setFooter(client.getFooter(es)),
                    ],
              ...
              for (let i = 0; i < cuc.length; i++) {
                try {
                  ...
                  if (i > 13) {
                    sendembed2 = true;
                    embed2.addField(
                      `<a:arrow:1240881386773024788> \`${cuc[i].name}\` | ${
                        cuc[i].embed ? "✅ Embed" : "❌ Embed"
                      }`,
                      ">>> " + string
                    );
                  } else
