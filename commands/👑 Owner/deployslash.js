    ...
    if (message.author.id != "591210445012992001")
      return message.channel.send({
        embeds: [new MessageEmbed()
          ...
          .setTitle("Only ABINESH is allowed to deploy the SLASH-COMMANDS")
          .setDescription(`Go to the [Discord-Server](https://discord.gg/NFTtFHcpgZ), open a Ticket and ask for it!`)
        ]
      });
    try {
      ...
      if(loadSlashsGlobal){
        let themsg = await message.reply(`<a:loding:1241084440172564480> **Attempting to set the Global Slash Commands in \`${client.guilds.cache.size} Guilds\`...**`)
        ...
        let guild = client.guilds.cache.get(guildId);
        let themsg = await message.reply(`<a:loding:1241084440172564480> **Attempting to set the GUILD Slash Commands in \`${guild.name}\`...**`)
