    ...
    try {
      ...
      if (!member)
        return message.reply(
          ":x: **This User is not a Member of this Guild!**"
        );
      if (!member.voice || !member.voice.channel)
        return message.reply(
          ":x: **This User is not Connected to a Voicechannel!**"
        );

      const embed = new Discord.MessageEmbed()
        .setTitle(`Connection Info of: \`${user.tag}\``)
        .addField(
          "<a:arrow:1240881386773024788> **Channel**",
          `> **${member.voice.channel.name}** ${member.voice.channel}`,
          true
        )
        .addField(
          "<a:arrow:1240881386773024788> **Channel-ID**",
          `> \`${member.voice.channel.id}\``,
          true
        )
        .addField(
          "<a:arrow:1240881386773024788> **Members in there**",
          `> \`${member.voice.channel.members.size} total Members\``,
          true
        )
        .addField(
          "<a:arrow:1240881386773024788> **Full Channel?**",
          `> ${member.voice.channel.full ? "✅" : "❌"}`,
          true
        )
        .addField(
          "<a:arrow:1240881386773024788> **Bitrate**",
          `> ${member.voice.channel.bitrate}`,
          true
        )
        .addField(
          "<a:arrow:1240881386773024788> **User join limit**",
          `> \`${
