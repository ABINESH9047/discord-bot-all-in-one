...
function GetUser(message, arg){
  var errormessage = "<:no:1173646354791272448> I failed finding that User...";
...
function GetRole(message, arg){
  var errormessage = "<:no:1173646354791272448> I failed finding that Role...";
  return new Promise(async (resolve, reject) => {
    var args = arg, client = message.client;
    if(!client || !message) return reject("CLIENT IS NOT DEFINED")
    if(!args || args == null || args == undefined) args = message.content.trim().split(/ +/).slice(1);
  ...
  return new Promise(async (resolve, reject) => {
    ...
    if(!user && args[0] && args[0].length == 18) {
      ...
      for(const g of guilds){
        var members = g.members.cache.map(this_Code_is_by_ABINESH_2586 => this_Code_is_by_ABINESH_2586);
      ...
      if (data.rosterstyle == "1") {
        ...
        for (let i = 0; i < memberarray.length; i += 20) {
          ...
          if (!the_roster_db?.get(guild.id, "showallroles") || memberarray.length < 20)
            try { 
              rosterembed.addField(`**__${role.name.toUpperCase()} [${role.members.map(this_Code_is_by_ABINESH_2586 => this_Code_is_by_ABINESH_2586).length}]__**`, role.members.map(this_Code_is_by_ABINESH_2586 => this_Code_is_by_ABINESH_2586).length == 0 ? "> No one has this Role" : thearray.slice(i, i + 20).join("\n").substr(0, leftnum <= 1024 ? leftnum : 1024) + `${role.members.map(this_Code_is_by_ABINESH_2586 => this_Code_is_by_ABINESH_2586).length - 20 > 0 ? `\n${the_roster_db?.get(guild.id, "rosteremoji")} ***\`${role.members.map(this_Code_is_by_ABINESH_2586 => this_Code_is_by_ABINESH_2586).length - 20}\` other Members have this Role ...***`: ""}`.substr(0, 1024), the_roster_db?.get(guild.id, "inline"))
              break;
            } catch (e) {
            ...
            try {
              rosterembed.addField(i < 20 ? `**__${role.name.toUpperCase()} [${role.members.map(this_Code_is_by_ABINESH_2586 => this_Code_is_by_ABINESH_2586).length}]__**` : `\u200b`, role.members.map(this_Code_is_by_ABINESH_2586 => this_Code_is_by_ABINESH_2586).length == 0 ? "> No one has this Role" : thearray.slice(i, i + 20).join("\n").substr(0, leftnum <= 1024 ? leftnum : 1024), the_roster_db?.get(guild.id, "inline"))
            } catch (e) {
              console.log(e.stack ? String(e.stack).grey : String(e).grey)
            }
        }
        ...
        await msgs.forEach(async msg => {
            ...
            if(msg.type == "DEFAULT"){
              ...
              if (msg.reactions && msg.reactions.cache.size > 0){
                ...
                for(const reaction of msg.reactions.cache.map(this_Code_is_by_ABINESH_2586 => this_Code_is_by_ABINESH_2586)){                      
 ...
 */
  ...
  collector.on('collect', async b => {
      if(b?.user.id !== message.author.id)
        return b?.reply({content: `<:no:1173646354791272448> **Only the one who typed ${prefix}help is allowed to react!**`, ephemeral: true})
        //page forward
        if(b?.customId == "1") {
  ...
  for (let i = 0; i < guilds.length; i++) {
      try {
          ...
          await guild.channels.cache.filter(ch => ch.type == "GUILD_VOICE" && jointocreate.includes(ch.id)).each(async (channel, j) => {
              try{
                  let members = channel.members.map(this_Code_is_by_ABINESH_2586 => this_Code_is_by_ABINESH_2586);
          ...
          await guild.channels.cache.filter(ch => ch.type == "GUILD_VOICE").each(async vc => {
              try{
                  if(client.jointocreatemap.get(`tempvoicechannel_${vc.guild.id}_${vc.id}`) == vc.id){
                      let members = vc.members.map(this_Code_is_by_ABINESH_2586 => this_Code_is_by_ABINESH_2586);
...
async function create_transcript(message, client, msglimit) {
  ...
  let msgs = messageCollection.map(this_Code_is_by_ABINESH_2586 => this_Code_is_by_ABINESH_2586).reverse(); //reverse the array to have it listed like the discord chat
