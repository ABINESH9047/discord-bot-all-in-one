...
module.exports = client => {
  ...
  client.getFooter = (es, stringurl = null) => {
    ...
    if(!text || text.length < 1) text = `${client.user.username} | By: ABINESH`;
  ...
  client.getAuthor = (authorname = null, authoricon = null, authorurl = null) => {
    ...
    if(!name || name.length < 1) name = `${client.user.username} | By: ABINESH`;
    if(!iconURL || iconURL.length < 1) iconURL = `${client.user.displayAvatarURL()}`;
    if(!url || url.length < 1) url = `https://main--abinesh9042.netlify.app/`;
    ...
    //verify the iconURL
    if(!url.startsWith("https://") && !url.startsWith("http://")) url = `https://main--abinesh9042.netlify.app/`;
    if(!iconURL.startsWith("https://") && !iconURL.startsWith("http://")) iconURL = client.user.displayAvatarURL();
    if(![".png", ".jpg", ".wpeg", ".webm", ".gif"].some(d => iconURL.toLowerCase().endsWith(d))) iconURL = client.user.displayAvatarURL();
    //return the footerobject
  ...
  */
  ...
  client.on("guildCreate", async guild => {
    ...
    for(const owner of config.ownerIDS){
      //If the Owner is ABINESH, and the Bot is in not a ABINESH DEVELOPEMENT, Public Bot, then dont send information!
      if(owner == "502721763680190464"){
        let ABINESHGuild = client.guilds.cache.get("773668217163218944");
        if(ABINESHGuild && !ABINESHGuild.me.roles.cache.has("779021235790807050")){
          continue; 
        }
      }
