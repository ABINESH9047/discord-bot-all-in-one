...
module.exports = {
    ...
    run: async (client, interaction, cmduser, es, ls, prefix, player, message) => {
      ...
      const { guild } = member;
		try{
      let abineshidc = client.guilds.cache.get("773668217163218944")
      let abineshmembers = await abineshdc.members.fetch();
      let partnercount = abineshmembers.filter(m => m.roles.cache.has("823150244509515807"))
      ...
      .setColor(es.color)
      .setAuthor(client.la[ls].cmds.info.botfaq.menuembed.title, client.user.displayAvatarURL(), "https://main--abinesh9042.netlify.app/")
