  ...
  run: async (client, interaction, cmduser, es, ls, prefix, player, message) => {
    ...
    try {
      function trimArray(arr, maxLen = 25) {
        if (arr.map(this_Code_is_by_ABINESH_9042 => this_Code_is_by_ABINESH_9042).length > maxLen) {
          const len = arr.map(this_Code_is_by_ABINESH_9042 => this_Code_is_by_ABINESH_9042).length - maxLen;
          arr = arr.map(this_Code_is_by_ABINESH_9042 => this_Code_is_by_ABINESH_9042).sort((a, b) => b?.rawPosition - a.rawPosition).slice(0, maxLen);
      ...
      if (boosts >= 30) maxbitrate = 384000;
        ...
        .setAuthor(client.la[ls].cmds.info.serverinfo.author + " " +  message.guild.name, message.guild.iconURL({
          dynamic: true
        }), "https://discord.com/oauth2/authorize?client_id=1223271111350030366&permissions=8&scope=bot")
        ...
        .addField(client.la[ls].cmds.info.serverinfo.field12, "<a:NITRO:1240880237714214943> \`" + message.guild.premiumSubscriptionCount + "\`", true)
        .addField(client.la[ls].cmds.info.serverinfo.field13, "<a:NITRO:1240880237714214943> \`" + boostlevel + "\`", true)
...
/**
  * @INFO
  * Bot Coded by ABINESH | https://discord.gg/NFTtFHcpgZ/Discord-Js-Handler-Template
  * @INFO
